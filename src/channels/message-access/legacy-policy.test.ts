...
describe("message-access legacy policy compatibility", () => {
  ...
  it.each([
    {
      ...
      expected: {
        effectiveAllowFrom: ["owner", "owner2", "owner3"],
        effectiveGroupAllowFrom: ["group:abc"],
      },
    },
    {
      name: "group fallback to DM allowlist",
      input: {
        allowFrom: [" owner "],
        groupAllowFrom: [],
      ...
      expected: {
        effectiveAllowFrom: ["owner", "owner2"],
        effectiveGroupAllowFrom: ["owner"],
      },
    },
    {
      name: "disabled group fallback",
      input: {
        allowFrom: ["owner"],
        groupAllowFrom: [],
        storeAllowFrom: ["paired-user"],
      ...
      expected: {
        effectiveAllowFrom: ["owner", "paired-user"],
        effectiveGroupAllowFrom: [],
      },
    },
    {
      name: "allowlist DM policy excludes pairing store",
      input: {
        allowFrom: ["+1111"],
        groupAllowFrom: ["group:abc"],
      ...
      expected: {
        effectiveAllowFrom: ["+1111"],
        effectiveGroupAllowFrom: ["group:abc"],
      },
    },
    {
      name: "open DM policy excludes pairing store",
      input: {
        allowFrom: ["owner"],
        groupAllowFrom: ["group:abc"],
        storeAllowFrom: ["paired-user"],
    ...
    {
      name: "pairing DM policy keeps explicit group fallback",
      input: {
        allowFrom: ["+1111"],
        groupAllowFrom: [],
        storeAllowFrom: ["+2222"],
      ...
      expected: {
        effectiveAllowFrom: ["+1111", "+2222"],
        effectiveGroupAllowFrom: ["+1111"],
      },
    },
  ])("resolves effective allowlists: $name", ({ input, expected }) => {
    expect(resolveEffectiveAllowFromLists(input)).toEqual(expected);
  });
  ...
  function expectParityCase(testCase: ParityCase) {
    ...
    expect(access.decision, testCase.name).toBe(testCase.expectedDecision);
    expect(reactionAllowed, `${testCase.name} reaction`).toBe(testCase.expectedReactionAllowed);
  }
  ...
  ])("keeps message/reaction policy parity: $name", (testCase) => {
    expectParityCase(testCase);
  });
  ...
  it.each(decisionCases)("$name", (testCase) => {
