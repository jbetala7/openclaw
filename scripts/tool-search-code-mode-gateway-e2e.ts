...
const FAKE_PLUGIN_ID = "tool-search-code-mode-e2e-fixture";
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      description: [
        ...
        "Used by the Tool Search Code Mode gateway E2E to prove a large plugin-owned tool catalog can be hidden from the model prompt and still called through the compact bridge.",
...
async function writeConfig(params: {
  ...
  let cfg = buildQaGatewayConfig({
    ...
    gatewayToken: "tool-search-code-mode-e2e",
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@openclaw/tool-search-code-mode-e2e-fixture",
        version: "0.0.0",
        type: "module",
        openclaw: {
          extensions: ["./index.js"],
    ...
    `${JSON.stringify(
      {
        ...
        name: "Tool Search Code Mode E2E Fixture",
        description: "Fake plugin with a large tool catalog for code-mode gateway validation.",
        contracts: {
        ...
        input: [
          {
            ...
            content: [
              {
                type: "input_text",
                text: `tool search code mode qa check target=${params.targetTool}`,
              },
            ],
          },
        ],
...
async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-tool-search-code-mode-"));
  ...
  try {
    ...
    assert(
      ...
      `expected code mode to expose fewer tools to provider: normal=${normal.providerDeclaredToolCount} code=${code.providerDeclaredToolCount}`,
    );
    assert(
      normal.providerRawBytes > code.providerRawBytes,
      `expected code mode request to be smaller: normal=${normal.providerRawBytes} code=${code.providerRawBytes}`,
    );
    assert(
